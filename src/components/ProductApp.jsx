...
import { checkTokenAlerts } from "../services/socialMarkets.js";
...
const NAV_GROUPS = [
  ["Meet", ["home", "discover", "network", "messages"]],
  ["Context", ["markets"]],
  ["Your FNF", ["rooms", "requests", "activity", "settings"]],
];
...
function HomeTab({ profile, crews, online, onTab, onEnter, onOpen, onCreate }) {
  ...
  return <div className="app-page">
    <header className="app-hero"><img className="app-hero__media" src="/assets/fnf-app-control-deck-v1.png" alt="" aria-hidden="true" /><div className="app-hero__veil" aria-hidden="true" /><div className="app-hero__copy"><span className="app-eyebrow">FNF / YOUR PRIVATE CREW DESK</span><h1>{profile ? `Your people are here, ${profile.display_name.split(" ")[0]}.` : "Find the people worth trading beside."}</h1><p>Private crews, real conversations and complete market context—without follower-count noise.</p><div className="app-hero__actions"><Button onClick={() => onTab("network")}><UserCircle />Meet people</Button><Button variant="secondary" onClick={onCreate}><Plus />Start a crew</Button></div></div></header>
...
export default function ProductApp({ session, crews, loading, onRefresh, onOpenCrew, onEnterRoom, onCreate, onSignOut }) {
  ...
  useEffect(() => {
    const check = () => checkTokenAlerts().catch(console.error);
    const receiveFallback = (event) => { const item = event.detail; setNotifications((current) => [item, ...current]); setToast(item); };
    window.addEventListener("fnf:market-alert", receiveFallback);
    const first = window.setTimeout(check, 4_000);
    const timer = window.setInterval(check, 60_000);
    return () => { window.clearTimeout(first); window.clearInterval(timer); window.removeEventListener("fnf:market-alert", receiveFallback); };
  }, [session.user.id]);
  ...
  return <div className="product-app">
    ...
    <aside className="app-sidebar"><div className="app-sidebar__brand"><button className="app-logo" onClick={() => go("home")}>FNF<span>●</span></button><div className="app-sidebar__status"><i />PRIVATE CREW OS</div></div><button className="app-market-command" onClick={() => setMarketQuick(true)}><span className="app-nav-key"><MagnifyingGlass /></span><span>Open market</span><kbd>⌘ K</kbd></button><nav>{NAV_GROUPS.map(([group, ids]) => <section className="app-nav-group" key={group}><span>{group}</span>{ids.map((id) => { const [, label, Icon] = TABS.find((item) => item[0] === id); return <button key={id} className={tab === id ? "active" : ""} onClick={() => go(id)}><span className="app-nav-key"><Icon weight={tab === id ? "fill" : "regular"} /></span><span>{label}</span>{id === "requests" && pending > 0 && <b>{pending}</b>}{id === "activity" && unread > 0 && <b>{unread}</b>}</button>; })}</section>)}</nav><div className="app-sidebar__account"><button onClick={() => go("settings")}><Avatar src={profile?.avatar_url} name={profile?.display_name || session.user.email || "FNF"} size={38} /><span><strong>{profile?.display_name || "FNF trader"}</strong><small>@{profile?.handle || "loading"}</small></span></button><button onClick={onSignOut} aria-label="Sign out"><SignOut /></button></div></aside>
