...
function MediaRoom({ room, crew, session, initialChart, onLeave, onCrewChanged }) {
  ...
  const chartPayload = (pair) => ({ symbol: pair.symbol, name: pair.name, quote: pair.quote, price: pair.price, change: pair.change, liquidity: pair.liquidity, volume: pair.volume, address: pair.address, tokenAddress: pair.tokenAddress, network: pair.network, chainId: pair.chainId, dex: pair.dex, url: pair.url });
  const postChart = async (pair) => sendRoomMessage(crew.id, `${pair.symbol} chart · $${pair.price < .01 ? pair.price.toPrecision(4) : pair.price.toFixed(2)} · ${pair.change >= 0 ? "+" : ""}${pair.change.toFixed(2)}% today`, "chart", chartPayload(pair));
  ...
  return <div className="room-workspace">
    ...
    <main className="room-main"><div className="room-stage">
      {share ? <div className="screen-stage"><AttachedTrack track={share.publication.track} /><span>{share.participant.name || "A crew member"} is sharing</span></div> : <MarketChart onShare={postChart} onDirectShare={directShare} members={members.filter((member) => member.user_id !== session.user.id)} externalChart={incomingChart} />}
